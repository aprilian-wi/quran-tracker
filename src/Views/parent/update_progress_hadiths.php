...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
    <div class="flex items-center gap-3">
        <div class="p-3 bg-white dark:bg-card-dark rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 text-orange-500">
            <span class="material-icons-round text-2xl">format_quote</span>
        </div>
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Update Hafalan Hadits</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Pembaruan progres hadits untuk <strong><?= h($child['name']) ?></strong></p>
        </div>
    </div>
...
<div class="bg-white dark:bg-card-dark rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 mb-8">
    <form method="POST" action="<?= BASE_URL ?>public/index.php?page=update_progress_hadiths">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <!-- Hadith Selection -->
            <div>
                <label for="hadith_id" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Pilih Hadits</label>
                <select name="hadith_id" id="hadith_id" class="block w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-orange-500 focus:border-orange-500 shadow-sm" required>
            ...
            <div>
                ...
                <select name="status" id="status" class="block w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-orange-500 focus:border-orange-500 shadow-sm" required>
        ...
        <div class="mb-6">
            ...
            <textarea name="note" id="note" class="block w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-orange-500 focus:border-orange-500 shadow-sm" rows="3" placeholder="Tambahkan catatan..."></textarea>
        </div>

        <div class="flex justify-end">
            <button type="submit" class="inline-flex items-center px-4 py-2.5 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-orange-500 hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-colors">
    ...
    <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            <div class="flex items-center gap-2">
                ...
                <select id="statusFilter" class="block w-full sm:w-auto rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-300 focus:ring-orange-500 focus:border-orange-500 py-1.5 pl-3 pr-8">
            ...
            <div class="flex items-center gap-2">
                ...
                <select id="updatedByFilter" class="block w-full sm:w-auto rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-300 focus:ring-orange-500 focus:border-orange-500 py-1.5 pl-3 pr-8">
                    <option value="">Semua</option>
        ...
        <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            <thead class="bg-slate-50 dark:bg-slate-800">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Hadits</th>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-6 flex items-start gap-4 mt-8">
        ...
        <div>
            ...
            <p class="text-sm text-blue-600 dark:text-blue-400">
                Belum ada data hadits yang tercatat untuk anak ini.
            </p>
        </div>
    </div>
<?php endif; ?>
