<?php
// src/Views/teacher/update_progress_books_pwa.php
...
<section class="pb-20">
    ...
    <form method="POST" action="<?= BASE_URL ?>public/index.php?page=update_progress_books"
        ...
        <input type="hidden" name="redirect_to" value="teacher/update_progress_books">
        ...
        <button type="submit"
            class="w-full bg-yellow-400 hover:bg-yellow-500 text-yellow-900 font-bold py-3.5 px-4 rounded-xl shadow-lg shadow-yellow-900/10 active:scale-95 transition-all flex items-center justify-center gap-2">
            ...
            foreach ($history as $entry):
                ...
                <div class="history-item relative overflow-hidden rounded-xl shadow-sm border border-gray-100 dark:border-gray-800"
                    ...
                    <div
                        ...
                        <!-- Status Stripe -->
                        <div
                            class="absolute left-0 top-0 bottom-0 w-1 <?= $entry['status'] === 'fluent' ? 'bg-green-500' : 'bg-amber-500' ?>">
                        </div>

                        <div class="flex justify-between items-start mb-2 pl-2">
                            <div>
                                <div class="flex items-center gap-2">
                                    <h4 class="font-bold text-gray-900 dark:text-white">Jilid <?= h($entry['volume_number']) ?>
                                        - <?= h($entry['title']) ?></h4>
                                </div>
                                <p class="text-sm text-text-sub-light dark:text-text-sub-dark mt-0.5">Halaman
                                    <?= $entry['page'] ?>
                                </p>
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        historyItems.forEach(item => {
            ...
            content.addEventListener('touchstart', (e) => {
                ...
                // Disable transition during drag for realtime feel
            ...
            content.addEventListener('touchmove', (e) => {
                ...
                if (diff < 0) {
                    // If we are scrolling vertically, don't intervene too much unless horizontal intention is clear
                    // Ideally prompt check angle. For now assume minimal complexity.
                    isSwiping = true;
                    currentX = Math.max(diff, -maxSwipe); // Limit swipe
                    content.style.transform = `translateX(${currentX}px)`;
                } else if (diff > 0 && item.dataset.open === 'true') {
                    // Closing
                    isSwiping = true;
                    currentX = Math.min(diff - 80, 0); // Start from open position (-80)
                    content.style.transform = `translateX(${currentX}px)`;
                }
            }, { passive: true });
            ...
            content.addEventListener('touchend', (e) => {
                ...
                // If dragged enough to left, snap open
