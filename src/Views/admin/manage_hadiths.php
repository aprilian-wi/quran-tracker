...
<div
    ...
    <div
        ...
        <div class="flex flex-col sm:flex-row w-full lg:w-auto gap-3">
            <a href="?page=admin/create_hadith"
            ...
            <thead class="bg-slate-50 dark:bg-slate-800/80">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider"
                        scope="col">Judul</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider"
                        scope="col">Teks Arab</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider"
                        scope="col">Terjemahan</th>
                    <th class="px-6 py-4 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-32"
                        scope="col">Aksi</th>
                </tr>
            </thead>
                ...
                <?php foreach ($hadiths as $hadith): ?>
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400 max-w-xs truncate"
                            title="<?= h($hadith['translation']) ?>">
                            <?= h($hadith['translation']) ?>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end gap-2">
                                <a href="?page=admin/edit_hadith&id=<?= $hadith['id'] ?>"
                                ...
                                <button onclick="confirmDelete(<?= $hadith['id'] ?>, '<?= h($hadith['title']) ?>')"
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
            ...
            <div class="bg-white dark:bg-card-dark px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-slate-500 dark:text-slate-400">
                                Apakah Anda yakin ingin menghapus hadits "<span id="itemTitle"
                                    class="font-bold"></span>"?
                            </p>
                            <p class="text-sm text-red-600 dark:text-red-400 mt-2 font-medium">
                                Peringatan: Ini juga akan menghapus semua catatan kemajuan yang terkait dengan hadits
                                ini.
                            </p>
                        </div>
                    </div>
            ...
            <div class="bg-slate-50 dark:bg-slate-800/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <form method="POST" action="?page=admin/delete_hadith" class="w-full sm:w-auto sm:ml-3">
                    <?= csrfInput() ?>
