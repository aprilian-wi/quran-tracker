...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Kelola Daftar Doa</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Manage short prayers and translations</p>
        </div>
    </div>
                ...
                <?php if (empty($prayers)): ?>
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-slate-500 dark:text-slate-400">
                            Tidak ada doa pendek ditemukan. Tambahkan doa pendek pertama Anda.
                        </td>
                    </tr>
                <?php endif; ?>
                ...
                <?php foreach ($prayers as $prayer): ?>
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-sm text-slate-800 dark:text-slate-200 font-amiri text-lg" dir="rtl">
                            <?= h($prayer['arabic_text']) ?>
                        </td>
                        <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400 max-w-xs truncate" title="<?= h($prayer['translation']) ?>">
                            <?= h($prayer['translation']) ?>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end gap-2">
                                <a href="?page=admin/edit_short_prayer&id=<?= $prayer['id'] ?>" class="text-amber-500 hover:text-amber-700 dark:text-amber-400 dark:hover:text-amber-300 p-1.5 rounded-lg hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors decoration-0" title="Edit">
                                    <span class="material-icons-round text-lg">edit</span>
                                </a>
                                <button onclick="confirmDelete(<?= $prayer['id'] ?>, '<?= h($prayer['title']) ?>')" class="text-rose-500 hover:text-rose-700 dark:text-rose-400 dark:hover:text-rose-300 p-1.5 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-900/20 transition-colors decoration-0" title="Hapus">
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white dark:bg-card-dark rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <div class="bg-white dark:bg-card-dark px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-slate-500 dark:text-slate-400">
                                Apakah Anda yakin ingin menghapus "<span id="itemTitle" class="font-bold"></span>"?
                            </p>
                            <p class="text-sm text-red-600 dark:text-red-400 mt-2 font-medium">
                                Peringatan: Ini juga akan menghapus semua catatan kemajuan yang terkait dengan doa pendek ini.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-slate-50 dark:bg-slate-800/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <form method="POST" action="?page=admin/delete_short_prayer" class="w-full sm:w-auto sm:ml-3">
                    <?= csrfInput() ?>
