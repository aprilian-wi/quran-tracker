...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <?php if (isset($_SESSION['success'])): ?>
            <div
                class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 p-4 rounded-lg flex items-center gap-3">
        ...
        <?php if (isset($_SESSION['error'])): ?>
            <div
                class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-800 dark:text-red-300 p-4 rounded-lg flex items-center gap-3">
                ...
                <form method="POST" action="<?= BASE_URL ?>public/index.php?page=edit_teacher" class="space-y-4">
                    ...
                    <div>
                        <label for="name" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nama
                            Lengkap *</label>
                        <input type="text" id="name" name="name" value="<?= h($teacher['name']) ?>" required
                    ...
                    <div>
                        <label for="phone" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">No.
                            HP *</label>
                        <input type="tel" id="phone" name="phone" value="<?= h($teacher['phone'] ?? '') ?>" required
                            class="block w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                    </div>
                ...
                <form method="POST" action="<?= BASE_URL ?>public/index.php?page=edit_teacher" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="password" id="new_password" name="new_password" required
                            class="block w-full rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm focus:border-cyan-500 focus:ring-cyan-500 sm:text-sm">
                    ...
                    <div class="pt-2">
                        <button type="submit"
                            class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 transition-colors">
            ...
            <div class="p-6">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <button onclick="document.getElementById('deleteModal').classList.remove('hidden')"
                        class="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg font-medium shadow-sm transition-colors flex items-center justify-center gap-2 whitespace-nowrap">
    ...
    <div class="lg:col-span-1">
        <div
            class="bg-white dark:bg-card-dark rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden sticky top-24">
            ...
            <div class="p-6 space-y-4">
                <div class="flex items-center justify-center py-4">
                    <div
                        class="w-24 h-24 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-400 font-bold text-3xl uppercase">
                ...
                <div class="space-y-3">
                    <div>
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Nama
                            Lengkap</label>
                        <p class="text-slate-900 dark:text-white font-medium"><?= h($teacher['name']) ?></p>
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">No. HP</label>
                        <p class="text-slate-900 dark:text-white break-all"><?= h($teacher['phone'] ?? '-') ?></p>
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Peran</label>
                        <p><span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-300 capitalize">
                                <?= h($teacher['role']) ?>
                            </span></p>
                    </div>
                    <div>
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog"
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
            class="inline-block align-bottom bg-white dark:bg-card-dark rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        ...
                        <div class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                            <p class="mb-2">Apakah Anda yakin ingin menghapus
                                <strong><?= h($teacher['name']) ?></strong>?
                            </p>
                            <p class="mb-4">Tindakan ini tidak dapat dibatalkan. Hubungan guru ke kelas akan diputuskan.
                            </p>
                                ...
                                <div class="flex">
                                    ...
                                    <div class="ml-3">
                                        <p class="text-xs text-amber-700 dark:text-amber-300">
                                            Ketik nama guru <strong><?= h($teacher['name']) ?></strong> untuk
                                            konfirmasi.
                                        </p>
                                    </div>
                                </div>
                            ...
                            <input type="text" id="confirmName"
                                class="block w-full rounded-md border-slate-300 dark:border-slate-600 shadow-sm focus:border-red-500 focus:ring-red-500 dark:bg-slate-700 dark:text-white sm:text-sm"
            ...
            <div class="bg-slate-50 dark:bg-slate-800/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <form method="POST" action="<?= BASE_URL ?>public/index.php?page=delete_teacher"
                    class="w-full sm:w-auto sm:ml-3">
                    <?= csrfInput() ?>
                    <input type="hidden" name="teacher_id" value="<?= $teacher['id'] ?>">
                    <button type="submit" id="confirmDeleteBtn" disabled
                        class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                        Hapus Guru
                    </button>
                </form>
                <button type="button" onclick="document.getElementById('deleteModal').classList.add('hidden')"
                    class="mt-3 w-full inline-flex justify-center rounded-md border border-slate-300 dark:border-slate-600 shadow-sm px-4 py-2 bg-white dark:bg-slate-700 text-base font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                    Batal
                </button>
            </div>
        </div>
