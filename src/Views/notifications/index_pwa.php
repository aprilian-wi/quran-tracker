...
if ($role === 'parent') {
    ...
    $notificationModel = new Notification($pdo);
    $notifications = $notificationModel->getByUser($user_id, 50, true);
}
...
<section class="mb-6 pt-2 px-1 flex items-center justify-between">
    <div class="flex items-center space-x-3">
        <a href="<?= ($role === 'teacher') ? BASE_URL . 'public/index.php?page=feed/index' : BASE_URL . 'public/index.php?page=parent/my_children&mode=pwa' ?>"
            class="text-text-main-light dark:text-white hover:text-primary transition-colors">
    ...
    <?php if (!empty($notifications)): ?>
        <button id="markAllReadBtn"
            class="text-xs font-medium text-primary dark:text-green-400 hover:text-primary-dark uppercase tracking-wide">
            Tandai Sudah Baca
        </button>
    <?php endif; ?>
</section>
...
<section class="space-y-4 pb-20 relative">
    ...
    <div id="empty-state"
        class="<?= empty($notifications) ? 'flex' : 'hidden' ?> flex-col items-center justify-center py-20 text-center space-y-4">
    ...
    <div id="notification-list" class="space-y-4">
        <?php if (!empty($notifications)): ?>
                <?php foreach ($notifications as $notif):
                    ...
                    ?>
                        <div class="notification-item <?= $bgColor ?> rounded-2xl p-4 border shadow-sm relative transition-all duration-500"
                    data-id="<?= $notif['id'] ?>">
                    <div class="flex items-start space-x-4">
                        <div
                            class="w-10 h-10 rounded-full bg-white dark:bg-gray-800 flex items-center justify-center text-primary dark:text-green-400 shadow-sm shrink-0">
