...
<div class="-mx-4 sm:mx-0 -mt-6">
    ...
    <div id="video-container" class="sticky top-0 z-40 bg-black w-full aspect-video shadow-lg">
        <iframe
            src="https://www.youtube.com/embed/<?= h($video['youtube_id']) ?>?autoplay=1&rel=0&modestbranding=1&iv_load_policy=3&playsinline=1&controls=1"
            title="<?= h($video['title']) ?>" frameborder="0"
        ...
        <button onclick="toggleFullscreen()"
            class="absolute bottom-4 right-4 bg-black/30 hover:bg-black/50 text-white w-10 h-10 rounded-full backdrop-blur-sm transition-all active:scale-90 flex items-center justify-center z-50 shadow-lg">
    ...
    <div class="px-4 py-4 bg-white dark:bg-surface-dark mb-2">
        ...
        <div
            class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
    ...
    <div class="px-4 pb-4">
        ...
        <div class="space-y-3">
            <?php foreach ($suggested as $item): ?>
                <a href="<?= BASE_URL ?>public/index.php?page=videos/watch&id=<?= $item['id'] ?>&mode=pwa"
                    class="flex bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden active:bg-gray-50 dark:active:bg-gray-900 transition-colors">
                    <div class="w-32 flex-shrink-0 relative">
                        <img src="https://img.youtube.com/vi/<?= h($item['youtube_id']) ?>/mqdefault.jpg"
                            alt="<?= h($item['title']) ?>" class="w-full h-full object-cover">
