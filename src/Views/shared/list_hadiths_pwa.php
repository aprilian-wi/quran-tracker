<?php
// src/Views/shared/list_hadiths_pwa.php
?>
<section>
    <div class="flex items-center space-x-2 mb-4 px-1">
        <span class="material-icons-round text-primary dark:text-green-400">format_quote</span>
        <h2 class="text-lg font-display font-bold text-text-main-light dark:text-white">Hadits Pilihan</h2>
    </div>
    ...
    <div class="space-y-4">
        <?php foreach ($hadiths as $hadith): ?>
            <div class="bg-surface-light dark:bg-surface-dark rounded-2xl shadow-card p-5 border border-gray-100 dark:border-gray-800">
                <h3 class="font-display font-bold text-gray-900 dark:text-white mb-3 border-b border-gray-100 dark:border-gray-700 pb-2"><?= h($hadith['title']) ?></h3>
    ...
    <?php if (empty($hadiths)): ?>
        <div class="bg-surface-light dark:bg-surface-dark rounded-2xl shadow-card p-6 text-center">
            <p class="text-text-sub-light dark:text-text-sub-dark">Tidak ada hadits.</p>
        </div>
    <?php endif; ?>
</section>
